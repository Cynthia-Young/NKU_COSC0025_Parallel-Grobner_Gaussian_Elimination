...
const int Columnnum = 562;          //矩阵列数
//const int Columnnum = 8399;
const int Rnum = 170;             //非零消元子
//const int Rnum = 6375;
const int Enum = 53;                //被消元行
//const int Enum =4535;
const int ArrayColumn = 18;          //8 = 254 / 32 ceiling  每一个位置用1bit表示，int型有32 bit，所以需要8个整形
//const int ArrayColumn = 263;
const int leftbit = 14;              //32 - (1 + 253 % 32) = 2
//const int leftbit = 17;                //32 - (1 + 43576 % 32) = 7
unsigned int R [Columnnum][ArrayColumn];  //消元子矩阵 最多不超过columnnum行
unsigned int E [Enum][ArrayColumn];       //被消元行矩阵 一共enum行
...
void Parallel_pthread(){
	...
	for(int t_id = 0; t_id < THREAD_NUM; t_id++)
	{
		...
		pthread_create(&handles[t_id], NULL, thread_xorset, (void *)(&param[t_id]));
	}
...
#pragma omp parallel for num_threads(THREAD_NUM) private(j) shared(E,R)
                ...
                {
                    float32x4_t Rfj = vld1q_f32(R[First[i]] + j);
                    float32x4_t Eij = vld1q_f32(E[i] + j);
                    Eij = veorq_f32(Rfj, Eij);
                    vst1q_f32(E[i] + j, Eij);
                }
...
int main()
{
    ...
    cout<<"Special Gauss, Parallel version, Enum: "<<Enum<<", Time: "<<time<<"ms"<<endl;
    Print();*/
    ...
    cout << "Special Gauss, Parallel version, Enum: " << Enum<< ", Time: " << (tail.tv_sec - head.tv_sec) * 1000.0 + (tail.tv_usec - head.tv_usec) / 1000.0 << "ms"<< endl;
    Print();
